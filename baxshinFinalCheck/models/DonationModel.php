...
class DonationModel
{
    ...
    function addMoney($phone_number, $amountOfMoney, $choosen_center)
    {
        $stmt = $this->conn->prepare("INSERT INTO moneyDonation(phone_number, amountOfMoney, choosen_center) VALUES (?, ?, ?)");
        $stmt->bind_param("sds", $phone_number, $amountOfMoney, $choosen_center);
    ...
    //Admin  add an event--------------------------------
    function addAboutUs($title, $description, $img1, $img2, $img3, $img4, $img5)
    {
        $stmt = $this->conn->prepare("INSERT INTO aboutUs(title ,aboutUs_description) VALUES (?, ?)");
        $stmt->bind_param("ss", $title, $description);
        if ($stmt->execute()) {
            $lastId = $this->conn->insert_id;
            if ($this->saveAboutUsImages($lastId, [$img1, $img2, $img3, $img4, $img5])) {
                $stmt->close();
                return true;
            }
        } else {
    ...
    private function saveAboutUsImages($aboutUsId, $images)
    {
        $stmt = $this->conn->prepare("INSERT INTO aboutUs_images (about_id, image_path) VALUES (?, ?)");
        $allSuccess = true;

        foreach ($images as $image) {
            if (!empty($image)) {
                $stmt->bind_param("is", $aboutUsId, $image);
                if (!$stmt->execute()) {
                    $allSuccess = false;
                }
            }
    ...
    //-----------add/delete center-admin-------------------------
    function addAdminCenter($image, $name, $city, $type, $needs, $description)
    {
        $stmt = $this->conn->prepare("INSERT INTO centers(image,name,city,type,needs,description) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssss", $image, $name, $city, $type, $needs, $description);
        if ($stmt->execute()) {
            $stmt->close();
            return true;
        } else {
    ...
    function deleteAdminCenter($addcenterid)
    {
        $stmt = $this->conn->prepare("DELETE FROM centers WHERE id = ?");
        $stmt->bind_param("i", $addcenterid);
        if ($stmt->execute()) {
            $stmt->close();
            return true;
        } else {
    ...
    //-----------add/delete profile-admin-------------------------
    function addAdminProfile($name, $age, $image, $needs, $description)
    {
        $stmt = $this->conn->prepare("INSERT INTO profiles(name,age,image,needs,description) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sisss", $name, $age, $image, $needs, $description);
        if ($stmt->execute()) {
            $stmt->close();
            return true;
        } else {
    ...
    {
        $aboutUsList = [];
        $sql = "SELECT * FROM aboutUs ORDER BY id desc LIMIT 5";
    ...
    {
        $stmt = $this->conn->prepare("SELECT user, pass FROM Admins WHERE user = ?");
    ...
    {
        $stmt = $this->conn->prepare("SELECT user FROM Admins WHERE user = ?");
